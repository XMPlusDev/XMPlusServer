...
func (c *Client) parseSubscriptionResponse(res *resty.Response, err error) (*SubscriptionResponse, error) {
	if err != nil {
		return nil, fmt.Errorf("subscription list request failed: %s", err)
	}
	if res.StatusCode() >= 400 {
		return nil, fmt.Errorf("subscription request error: %v", err)
	}
	return res.Result().(*SubscriptionResponse), nil
}
...
func (c *Client) GetSubscriptionList() (*[]SubscriptionInfo, error) {
	...
	return c.ParseSubscriptionList(subscriptionsListResponse)
}
...
func (c *Client) ReportTraffic(subscriptionTraffic *[]SubscriptionTraffic) error {
	...
	postData := &PostData{Key: c.Key, Data: data}
	res, err := c.client.R().
		SetBody(postData).
		SetPathParam("serverId", strconv.Itoa(c.NodeID)).
		ForceContentType("application/json").
