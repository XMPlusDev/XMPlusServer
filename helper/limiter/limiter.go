...
func (l *Limiter) GetSubscriptionLimiter(tag string, email string, ip string, address string) (limiter *rate.Limiter, isSpeedLimited bool, Reject bool) {
	if value, ok := l.InboundInfo.Load(tag); ok {
		var (
			SpeedLimit  uint64 = 0
			ipLimit, uid int
